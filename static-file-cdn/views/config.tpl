      ...
      <div class="col-md-6">
        ...
        $form = Option::form('cdn', 'CDN 配置', function($form)
        {
          $form->text('cdn_address', 'CDN 地址')->hint('不要以 / 结尾')->description(
            '填写的 CDN 地址必须是 <code>/resources/assets</code> 目录的镜像，此目录下的所有文件都将会从 CDN 加载。<br>'.
            '<b>测试方法</b>：检查 <code>{你应该填写的地址}/dist/js/app.js</code> 是否能够访问。设置无法访问的 CDN 造成的问题本插件<b>概不负责</b>（如果真的手贱了在 plugins 目录下删除本插件即可）。<br>'.
